...
contract SwapMining is Ownable {
    ...
    function getUserReward(uint256 _pid) public view returns (uint256, uint256){
        ...
        if (user.quantity > 0) {
            uint256 blockReward = getDexReward(pool.lastRewardBlock);
            uint256 dexReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
            userSub = userSub.add((pool.allocDexAmount.add(dexReward)).mul(user.quantity).div(pool.quantity));
        }
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        if (outputToken == anchorToken) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (IDexFactory(factory).getPair(outputToken, intermediate) != address(0) && IDexFactory(factory).getPair(intermediate, anchorToken) != address(0)) {
